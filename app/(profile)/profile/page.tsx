...
export default async function ProfilePage() {
  const cookieStore = await cookies(); // server-only cookie access
  const accessToken = cookieStore.get("access_token");

  const user = await getUser();

  //no token or not logged in → redirect to login page
  if (user.username === null || user.isLoggedIn === false || !accessToken)
    redirect("/sign-in");

  // fetch full profile that needs token (orders, createdAt, etc.)
  const userData = await getUserData(user.userId, accessToken.value);

  // used to render "member since" in a friendly format
      ...
      <div className="col-span-2 bg-gray-50 p-6 md:p-10 rounded">
        <div className="flex flex-col">
          ...
          {userData.orders && userData.orders.length > 0 ? (
            ...
            // empty state when no orders are present
